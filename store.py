...
def save_youtube_to_csv(videoId, scale, notes, tags, filename='music_data.csv'):
	save_to_csv(youtube_id_to_url(videoId), scale, notes, tags, filename=filename)

def save_to_csv(url, scale, notes, tags, filename='music_data.csv'):
	if url != "" and scale != "":
...
def get_entries_for_scale(scale, filename='music_data.csv'):
    entries = []
    with open(filename, mode='r') as file:
        reader = csv.reader(file)
        for row in reader:
